...
export default function ReportsPage() {
  ...
  const filtered = reports.filter((r) => {
    const matchesQuery =
      ...
      r.club.toLowerCase().includes(query.toLowerCase());
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1.5 text-sm text-ink-soft">
              Review monthly activities, budgets, and event reports submitted by
              clubs.
            </p>
          </div>
          <Button leftIcon="Upload" onClick={handleUpload} magnetic>
            Submit Report
          </Button>
        </div>
      </FadeIn>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <Badge
                      tone={
                        reportTypeTone[r.type as keyof typeof reportTypeTone]
                      }
                    >
                      {r.type}
                    </Badge>
                    <Badge
                      tone={
                        reportStatusTone[
                          r.status as keyof typeof reportStatusTone
                        ]
                      }
                    >
                      {r.status}
