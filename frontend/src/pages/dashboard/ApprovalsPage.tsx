...
export default function ApprovalsPage() {
  ...
  const filtered = approvals.filter((a) => {
    const matchesQuery =
      ...
      a.submittedBy.toLowerCase().includes(query.toLowerCase());
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1.5 text-sm text-ink-soft">
              Review and approve club proposals for events, budgets, and
              projects.
            </p>
          </div>
        </div>
      ...
      <StaggerGroup className="grid gap-6">
        {filtered.length > 0 ? (
          filtered.map((a) => {
            const Icon =
              typeIcons[a.type as keyof typeof typeIcons] || FileText;
            const colorClass =
              typeColors[a.type as keyof typeof typeColors] ||
          ...
          <FadeIn>
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border-soft p-12 text-center bg-white/50">
              ...
              <p className="text-xs text-ink-soft">
                Check back later or change your search/filter settings.
              </p>
            </div>
          </FadeIn>
        )}
