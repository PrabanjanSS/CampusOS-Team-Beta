...
export default function ProjectsPage() {
  ...
  // Tab control
  const [activeTab, setActiveTab] = useState<"showcase" | "tracker">(
    "showcase",
  );
  ...
  const filteredShowcase = useMemo(() => {
    return showcaseProjects.filter((project) => {
      const matchesSearch =
        ...
        project.description
          .toLowerCase()
          .includes(showcaseSearch.toLowerCase());
  ...
  const filteredTracker = projects.filter((p: any) =>
    p.name.toLowerCase().includes(trackerQuery.toLowerCase()),
  );
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-soft">
              Explore and track active club projects.
            </p>
          </div>
            ...
            <div className="space-y-4">
              ...
              {filteredShowcase.length === 0 ? (
                <div className="card-surface p-12 text-center bg-white/70">
                  ...
                  <p className="mt-1 text-sm text-ink-soft">
                    Try another search query or filter by club.
                  </p>
                </div>
              ) : (
        ...
        <div className="space-y-6">
          <FadeIn delay={0.08}>
            <SearchBar
              ...
              placeholder="Search active tracker projects…"
        ...
        >
          <div className="space-y-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-soft">
                    {selectedProject.tag || "Club Project"}
                  </p>
                </div>
              </div>
              <Badge
                tone={
                  statusTone[selectedProject.status as keyof typeof statusTone]
                }
                dot
              >
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-ink-soft/90 leading-relaxed">
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  .slice(
                    ...
                    selectedProject.members
                      ? Math.min(selectedProject.members, 3)
                      : 3,
                  )
                  .map((member) => (
                    <div
                ...
                {(() => {
                  ...
                  const openSeatsCount = Math.max(
                    0,
                    totalSeats - activeMockCount,
                  );
                  if (openSeatsCount <= 0) return null;

                  const hasRequested = joinRequests.includes(
                    selectedProject.id,
                  );
                  ...
                  return Array.from({ length: openSeatsCount }).map((_, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between rounded-xl border border-dashed border-border-soft/80 bg-cream-100/30 p-3"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            localStorage.setItem(
                              "campusos_join_requests",
                              JSON.stringify(updated),
                            );
                            toast({
                              title: "Request Sent Successfully",
                              description:
                                "Your request to join the project team has been recorded and will be sent to the lead.",
                              variant: "success",
                            });
                          }}
                        >
