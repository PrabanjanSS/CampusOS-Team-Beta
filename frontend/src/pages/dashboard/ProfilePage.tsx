...
const coverThemes = {
  navy: {
    type: "gradient",
    label: "Classic navy",
    className: "from-navy via-navy-600 to-navy-400",
  },
  violet: {
    type: "gradient",
    label: "Violet haze",
    className: "from-violet-800 via-violet-600 to-indigo-400",
  },
  emerald: {
    type: "gradient",
    label: "Emerald",
    className: "from-emerald-800 via-teal-600 to-cyan-400",
  },
  sunset: {
    type: "gradient",
    label: "Sunset",
    className: "from-rose-700 via-orange-500 to-amber-300",
  },
  tech: {
    type: "image",
    label: "Technology",
  ...
  abstract: {
    type: "image",
    label: "Abstract Flow",
    url: "https://images.pexels.com/photos/1242348/pexels-photo-1242348.jpeg?auto=compress&cs=tinysrgb&w=600",
  },
} as const;
...
export default function ProfilePage() {
  ...
  const [selectedCover, setSelectedCover] = useState<keyof typeof coverThemes>(
    user?.coverTheme ?? "navy",
  );
  ...
  const syncProfileToRegistry = (
    updatedCertificates: any[],
    profilePatch?: Partial<any>,
    skillsPatch?: string[],
  ) => {
    if (!user) return;
    ...
    const currentBio =
      profilePatch?.bio !== undefined
        ...
        : bio || user.bio || "";
    const currentDept =
      profilePatch?.department !== undefined
        ? profilePatch.department
        : department || user.department || "Computer Science";
    const currentYear =
      profilePatch?.year !== undefined
        ? profilePatch.year
        : year || user.year || "2nd Year";
    const currentClub =
      profilePatch?.club !== undefined
        ? profilePatch.club
        : club || user.club || "Developers Club";
    const currentSkills =
      skillsPatch !== undefined ? skillsPatch : user.skills || [];
    ...
    profilesMap[user.name] = {
      ...
      avatarUrl:
        user.avatarUrl ||
        "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=256&h=256&fit=crop",
  ...
  const save = () => {
    ...
    syncProfileToRegistry(user?.certificates || [], {
      bio,
      department,
      year,
      club,
  ...
  const saveCover = () => {
    ...
    if (uploadUrl) {
      ...
      toast({
        title: "Cover updated",
        description: `${coverThemes[selectedCover].label} is now your profile cover.`,
        variant: "success",
      });
    }
    setCoverOpen(false);
  ...
  const activeBannerPreview =
    coverTab === "upload"
      ? uploadedBanner
      : coverTab === "url"
        ? customBannerUrl
        : coverThemes[selectedCover]?.type === "image"
          ? (coverThemes[selectedCover] as any).url
          : "";
    ...
    <div className="space-y-6">
        ...
        <div className="overflow-hidden rounded-3xl border border-border-soft bg-white shadow-card">
          <div className="relative h-44 sm:h-56 overflow-hidden bg-navy">
            {user?.coverImage ? (
              ...
              <img
                src={(coverThemes[user?.coverTheme ?? "navy"] as any).url}
                className="h-full w-full object-cover"
                alt="Profile Cover"
              />
            ) : (
              <div
                className={`h-full w-full bg-gradient-to-r ${(coverThemes[user?.coverTheme ?? "navy"] as any).className ?? (coverThemes.navy as any).className}`}
              />
            )}
            ...
            <button
              ...
              className="absolute right-4 top-4 inline-flex items-center gap-1.5 rounded-lg bg-white/15 px-3 py-1.5 text-xs font-semibold text-white backdrop-blur transition-colors hover:bg-white/25"
            >
          ...
          <div className="relative px-6 pb-6 pt-4">
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
                ...
                >
                  <Avatar
                    ...
                    size="xl"
                    ring
                    className="shadow-lift ring-4 ring-white"
                  />
                </motion.div>
                <div className="pb-1">
            ...
            {/* Bio */}
            {user?.bio && (
              <p className="mt-5 max-w-2xl text-sm leading-relaxed text-ink-soft text-center sm:text-left mx-auto sm:mx-0">
                {user.bio}
              </p>
            )}
            ...
            <div className="mt-5 flex flex-wrap justify-center sm:justify-start gap-5 text-sm text-ink-soft">
              ...
              <span className="inline-flex items-center gap-1.5">
                <CalendarDays className="h-4 w-4" /> {user?.year}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <MapPin className="h-4 w-4" /> {user?.club}
              </span>
            </div>
          </div>
        </div>
              ...
              <div className="relative space-y-5 pl-2">
                ...
                {mockTimeline.map((t, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${c.bg} ${c.color} ring-4 ring-cream`}
                      >
      ...
      <FadeIn delay={0.1}>
        <Card>
          <CardHeader
            title="Badges"
            subtitle="Recognition earned across the platform"
          />
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={save} leftIcon="Check">
              Save changes
            </Button>
          </div>
        </div>
      ...
      <Modal
        ...
        description="Customise your profile header banner"
        size="md"
      >
        ...
        {activeBannerPreview ? (
          ...
          <div
            className={`mb-4 h-24 w-full overflow-hidden rounded-2xl bg-gradient-to-br ${(coverThemes[selectedCover] as any).className ?? "from-navy to-navy-400"} shadow-sm`}
          />
        )}
            ...
            {Object.entries(coverThemes).map(([key, cover]) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setSelectedCover(key as keyof typeof coverThemes)
                  }
                ...
                >
                  {cover.type === "image" ? (
                    <img
                      src={(cover as any).url}
                      className="block h-14 w-full object-cover"
                      alt={cover.label}
                    />
                  ) : (
                    <span
                      className={`block h-14 bg-gradient-to-br ${(cover as any).className}`}
                    />
                  )}
                  {isSelected && (
                  ...
                  <span className="block truncate px-1.5 py-1.5 text-[0.65rem] font-semibold text-ink">
                    {cover.label}
                  </span>
                </button>
              );
          ...
          <div className="space-y-3">
            <div
              ...
              onDrop={(e) => {
                ...
                handleBannerFileChange(e.dataTransfer.files[0] ?? null);
              }}
              ...
              {uploadedBanner ? (
                <>
                  <img
                    src={uploadedBanner}
                    alt="Uploaded"
                    className="h-20 w-full rounded-xl object-cover shadow-md"
                  />
                ...
                <>
                  ...
                  <p className="text-[0.7rem] text-ink-soft/60">
                    JPG, PNG, GIF — best at 1200×300px, max 5 MB
                  </p>
                </>
              )}
            </div>
            <input
              ...
              className="hidden"
              onChange={(e) =>
                handleBannerFileChange(e.target.files?.[0] ?? null)
              }
            />
            {uploadedBanner && (
              <Button
        ...
        {coverTab === "url" && (
          <div className="space-y-3">
            ...
            <p className="text-xs text-ink-soft">
              Paste a direct link to any publicly accessible image. Best
        ...
        <div className="mt-6 flex gap-3">
          ...
          <Button className="flex-1" leftIcon="Check" onClick={saveCover}>
            Save cover
          </Button>
        </div>
      </Modal>
          ...
          <div className="text-center space-y-4 py-2">
            ...
            <div>
              ...
              <p className="text-xs text-ink-soft mt-1">
                Earned on {formatDate(activeAchievement.date)}
              </p>
            </div>
          ...
          <div className="text-center space-y-4 py-2">
            ...
            <div>
              ...
              <p className="text-xs text-ink-soft/70 mt-0.5">
                Date: {formatDate(activeCertificate.date)}
              </p>
            </div>
            ...
            <div className="text-xs text-left text-ink-soft/80 bg-cream-100/55 p-4 rounded-xl border border-border-soft space-y-2">
              <p>
                <span className="font-semibold text-ink">Credential ID:</span>{" "}
                UC-{activeCertificate.id.toUpperCase()}-
              ...
              <p className="pt-1 text-[11px] leading-relaxed">
                ...
                requirements and peer-review checks within the CampusOS
                ecosystem.
              </p>
            </div>
            <div className="flex justify-end pt-2">
            ...
            <div className="border-t border-border-soft/60 pt-3">
              ...
              <div className="relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border-soft bg-cream-100/20 px-4 py-5 text-center transition hover:bg-cream-100/40 cursor-pointer">
                ...
                <p className="text-xs font-bold text-ink">
                  Click or drag certificate image
                </p>
              ...
              {certFileUrl && (
                <div className="mt-3 relative h-28 w-full overflow-hidden rounded-xl border border-border-soft">
                  <img
                    src={certFileUrl}
                    className="h-full w-full object-contain bg-cream-100/30"
      ...
      {isAddSkillOpen && (
        ...
        >
          <form onSubmit={handleAddSkillSubmit} className="space-y-4 text-left">
            ...
            {/* Popular presets */}
            <div>
              <label className="label-base text-[11px] uppercase tracking-wider text-ink-soft/75">
                Popular skills
              </label>
...
function ProfileStat({
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      className="card-surface flex items-center gap-3.5 p-4 transition-shadow hover:shadow-lift"
    >
