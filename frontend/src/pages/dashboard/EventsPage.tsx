...
export default function EventsPage() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("campusos_event_registered", loadRegs);
  }, []);
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-soft">
              Browse, register, and manage club events.
            </p>
          </div>
          {canCreate && (
            <Button
      ...
      <StaggerGroup className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((e: any) => (
          <StaggerItem key={e.id}>
            <motion.div
              whileHover={{ y: -6 }}
              className="card-surface group overflow-hidden transition-shadow hover:shadow-lift"
            >
              <div className="relative h-32 overflow-hidden bg-gradient-to-br from-navy to-navy-400">
                ...
                <div className="absolute left-4 top-4">
                  <Badge
                    tone={statusTone[e.status as keyof typeof statusTone]}
                    dot
                  >
                    {e.status}
                  </Badge>
              ...
              <div className="p-5">
                ...
                {(() => {
                  const isRegistered = registeredList.includes(
                    String(e.id || e.title),
                  );
                  ...
                  return (
                    <Button
                      variant={
                        isRegistered && !isLive ? "outline" : "secondary"
                      }
