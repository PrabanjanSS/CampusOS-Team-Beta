...
import {
  EventActionModal,
  type EventActionDetails,
} from "../../components/events/EventActionModal";
import {
  mockBlogPosts,
  mockAnnouncements,
  mockLeaderboard,
...
export default function MemberDashboard() {
  const { user } = useAuth();
  const [selectedEvent, setSelectedEvent] = useState<EventActionDetails | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("campusos_event_registered", update);
  }, []);
      ...
      <FadeIn>
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <div className="flex gap-2.5">
            <Link to="/app/events">
              <Button variant="secondary" leftIcon="Search" size="md">
                Find Events
              </Button>
            </Link>
            <Link to="/app/profile">
              <Button leftIcon="UserRound" size="md" magnetic>
                My Profile
              </Button>
            </Link>
          </div>
          ...
          <FadeIn delay={0.14}>
            <Card>
              <CardHeader
                ...
                action={
                  <Badge tone="navy">{registeredEvents.length} upcoming</Badge>
                }
              />
              <div className="space-y-3">
        ...
        <div className="space-y-6">
              ...
              <div className="flex flex-col items-center p-2 text-center">
                ...
                <div className="mt-4 grid w-full grid-cols-3 gap-2 border-t border-border-soft pt-4">
                  {[
                    ...
                    {
                      label: "Certs",
                      val: `${user?.certificates?.length ?? 0}`,
                      icon: Award,
                    },
                  ].map((s) => (
                    <div
            ...
            <Card>
              ...
              <div className="space-y-2.5">
                {mockLeaderboard.slice(0, 5).map((p, i) => (
                  ...
                  >
                    <span
                      className={`w-6 text-center text-xs font-bold ${p.rank <= 3 ? "text-[#b07314]" : "text-ink-soft"}`}
                    >
                      {p.rank}
                    </span>
          ...
          <FadeIn delay={0.18}>
            <Card>
              <CardHeader
                title="Announcements"
                action={<Bell className="h-4 w-4 text-ink-soft" />}
              />
              <div className="space-y-3">
                {announcements.slice(0, 2).map((a: any, i: number) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${a.priority === "high" ? "bg-danger" : a.priority === "normal" ? "bg-warning" : "bg-ink-soft/40"}`}
                      />
                    ...
                    <p className="mt-2 text-[0.68rem] text-ink-soft/70">
                      {a.author} · {a.date}
                    </p>
                  </motion.div>
                ))}
